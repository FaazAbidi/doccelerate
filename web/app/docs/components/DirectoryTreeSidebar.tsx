...
interface DirectoryTreeSidebarProps {
  ...
  /**
   * Whether the user has GitHub connected (controls commit prompt visibility)
   */
...
export function DirectoryTreeSidebar({ 
  ...
  // Show commit prompt when there are modified files AND user has GitHub connection
  const shouldShowCommitPrompt = modifiedFilesInfo.hasModifiedFiles && hasGithubConnection;

  return (
    <>
      {/* Commit prompt or GitHub connection encouragement */}
      ...
      {/* GitHub connection prompt for modified files when not connected */}
      {modifiedFilesInfo.hasModifiedFiles && !hasGithubConnection && (
        <div className={`${positioningClasses} -translate-y-[5px] transition-all duration-200 ease-in-out ${isClosing ? 'opacity-0 translate-x-[-20px]' : 'opacity-100 translate-x-0'}`}>
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-[20px] px-4 py-3 w-80 mb-2">
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1">
                <p className="text-caption text-blue-700 font-medium">
                  {modifiedFilesInfo.modifiedCount} file{modifiedFilesInfo.modifiedCount !== 1 ? 's' : ''} modified
                </p>
                <p className="text-xs text-blue-600">Connect GitHub to commit your changes</p>
                <div className="mt-2">
                  <button
